...
```bash
cd <topview-ai-plugin-root>/tools/seo-browser-uploader
...
```bash
<topview-ai-plugin-root>/tools/seo-browser-uploader/.venv/bin/social-upload
```
...
3. If Chrome is not available, launch a debug Chrome session with the bundled `tools/seo-browser-uploader/scripts/start_chrome_debug.sh` and ask the user to log in.
